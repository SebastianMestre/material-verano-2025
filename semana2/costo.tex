  ...
  \begin{itemize}
    ...
    \item Ejemplo: cuando \(N\) es grande
      \[
        N^2 + N \sim N^2.
      \]
    \item En práctica competitiva el límite de tiempo suele ser bastante holgado, \href{https://codeforces.com/contest/2109/status}{incluso \textbf{20x más de lo necesario}}.
      \begin{itemize}
...
\begin{frame}[fragile]{Operaciones básicas: \(O(1)\)}
  \begin{itemize}
    \item Muchas operaciones simples (y no tan simples) tienen \textbf{costo constante} \(O(1)\).
  \end{itemize}
  \begin{block}{Ejemplo}
...
\begin{frame}[fragile]{Bucles anidados: multiplicar costos}
  \begin{itemize}
    \item Si tenemos algo dentro de un bucle, el costo se \textbf{multiplica} por la cantidad de operaciones.
  \end{itemize}
  \begin{block}{Ejemplo}
  \begin{lstlisting}[style=customc]
for (int i = 0; i < N; ++i) {   // O(N) iteraciones
